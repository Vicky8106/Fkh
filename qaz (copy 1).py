...
PWD = '2580'
API_KEY = 'Csnv5q4a'
token = 'M3MK3DFILLIUB6U2CS7YUY2HHY'
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    data = obj.generateSession(USER_NAME,PWD,pyotp.TOTP(token).now())
    ...
    if data is not None:
        if 'data' in data and data['data'] is not None:
            ...
            print("Error: data['data'] is None.")
            # Handle the situation when data['data'] is None
    else:
        print("Error: data is None.")
        # Handle the situation when data is None
    ...
    def sltp():
        while True:
            if datetime.strptime('09:15','%H:%M').time()<=datetime.now().time():
                break
            time.sleep(0.05)
        while datetime.strptime('09:15','%H:%M').time()<=datetime.now().time()<=datetime.strptime('16:00','%H:%M').time():
    ...
    def round5(x):
        return round(int(x/0.05)*0.05,2)
        ...
        if pod['status']==True and pod['data']!=None:
            ...
            if TokenInfo['symbol'].iloc[0] not in p["tradingsymbol"].values:
                if var10.get()==1 and var11.get()==1:
                    if STOPLOSS.get()=='':
                        sl_tp[sp]=[TokenInfo['symbol'].iloc[0],'BUY',round((float(pris())-round5(float(pris())*0.01)),2),float(TARGET.get()),freeze(var5,var6)]
                    else:
                    ...
                    if STOPLOSS.get()=='':
                        sl_tp[sp]=[TokenInfo['symbol'].iloc[0],'BUY',round((float(pris())-round5(float(pris())*0.01)),2),None,freeze(var5,var6)]
                    else:
    ...
    def sellbf():
        ...
        if pod['status']==True and pod['data']!=None:
            ...
            if TokenInfo['symbol'].iloc[0] not in p["tradingsymbol"].values:
                if var10.get()==1 and var11.get()==1:
                    if STOPLOSS.get()=='':
                        sl_tp[sp]=[TokenInfo['symbol'].iloc[0],'SELL',round((float(pris())+round5(float(pris())*0.01)),2),float(TARGET.get()),freeze(var5,var6)]
                    else:
                        sl_tp[sp]=[TokenInfo['symbol'].iloc[0],'SELL',float(STOPLOSS.get()),float(TARGET.get()),freeze(var5,var6)]
                elif var10.get()==1 and var11.get()==0:
                    if STOPLOSS.get()=='':
                        sl_tp[sp]=[TokenInfo['symbol'].iloc[0],'SELL',round((float(pris())+round5(float(pris())*0.01)),2),None,freeze(var5,var6)]
                    else:
        ...
        for symb in bskt:
            ...
            for s in sl_tp:
                if bskt[symb][0]==sl_tp[s][0] and sl_tp[s][2]==0:
                    ...
                    if sl_tp[s][1]=='BUY':
                        sl_tp[s][2]=round((ltp-(ltp*0.01)),1)
                        break
                    elif sl_tp[s][1]=='SELL':
                        sl_tp[s][2]=round((ltp+(ltp*0.01)),1)
                        break
